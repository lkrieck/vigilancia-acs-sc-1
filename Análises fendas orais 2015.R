...
fendas.muni.resid2015 <- anom.grupos2015 %>% 
...
fendas.macro.resid2015 <- anom.grupos2015 %>% 
  filter(GrupoCID10 == "Fendas orais") %>% 
  group_by(CodigoMacrorregiaoSaude, NumeroIdentificacao) %>%
  count() %>%
  group_by(CodigoMacrorregiaoSaude) %>% 
...
fendas.reg.resid2015 <- anom.grupos2015 %>%  
  filter(GrupoCID10 == "Fendas orais") %>%
  group_by(CodigoRegiaoSaude, NumeroIdentificacao) %>%
  count() %>%
  group_by(CodigoRegiaoSaude) %>% 
...
fendas.muni.nasc2015 <- anom.grupos2015 %>% 
  filter(GrupoCID10 == "Fendas orais") %>%
  group_by(CodigoMunicipioNascimento, NumeroIdentificacao) %>%
  count() %>%
  group_by(CodigoMunicipioNascimento) %>%
...
fendas.estab2015 <- anom.grupos2015 %>% 
  filter(GrupoCID10 == "Fendas orais") %>%
...
prev.fendas.muni.resid2015 <- fendas.muni.resid2015 %>% 
  left_join(nv.muni.resid2015, fendas.muni.resid2015, by = "code_muni")

#calculando prevalencia
epiprev.fendas.muni.resid2015 <-epi.prev(prev.fendas.muni.resid2015$n, prev.fendas.muni.resid2015$nasc_vivos, se = 0.95, sp = 0.95, units = 10000)

#criando objeto com prevalencias e IC
epiprev.fendas.muni.resid2015_2 <- epiprev.fendas.muni.resid2015[["ap"]]

#unindo os dados de prevalencia
prev.fendas.muni.resid2015 <- prev.fendas.muni.resid2015 %>% 
  bind_cols(epiprev.fendas.muni.resid2015_2) %>% 
...
#unindo os dados de nascidos vivos e anomalias
prev.fendas.macro.resid2015  <- fendas.macro.resid2015 %>% 
  left_join(nv.macro.resid2015, fendas.macro.resid2015, by = "code_health_marcroregion")

#calculando prevalencia
epiprev.fendas.macro.resid2015 <-epi.prev(prev.fendas.macro.resid2015$n, prev.fendas.macro.resid2015$nasc_vivos, se = 0.95, sp = 0.95, units = 10000)

#criando objeto com prevalencias e IC
epiprev.fendas.macro.resid2015_2 <- epiprev.fendas.macro.resid2015[["ap"]]

#unindo os dados de prevalencia
prev.fendas.macro.resid2015 <- prev.fendas.macro.resid2015 %>% 
  bind_cols(epiprev.fendas.macro.resid2015_2) %>% 
  mutate(est = round(est, 2))
...
#unindo os dados de nascidos vivos e anomalias
prev.fendas.reg.resid2015 <- fendas.reg.resid2015 %>% 
  left_join(nv.reg.resid2015, fendas.reg.resid2015, by = "code_health_region")

#calculando prevalencia
epiprev.fendas.reg.resid2015 <-epi.prev(prev.fendas.reg.resid2015$n, prev.fendas.reg.resid2015$nasc_vivos, se = 0.95, sp = 0.95, units = 10000)

##criando objeto com prevalencias e IC
epiprev.fendas.reg.resid2015_2 <- epiprev.fendas.reg.resid2015[["ap"]]

#unindo os dados de prevalencia
prev.fendas.reg.resid2015 <- prev.fendas.reg.resid2015 %>% 
  bind_cols(epiprev.fendas.reg.resid2015_2) %>% 
  mutate(est = round(est, 2))
...
#unindo os dados de nascidos vivos e anomalias
prev.fendas.muni.nasc2015 <- fendas.muni.nasc2015 %>% 
  left_join(nv.muni.nasc2015, fendas.muni.nasc2015, by = "code_muni") %>% 
  filter(str_detect(code_muni,"42"))

#calculando prevalencia
epiprev.fendas.muni.nasc2015 <-epi.prev(prev.fendas.muni.nasc2015$n, prev.fendas.muni.nasc2015$nasc_vivos, se = 0.95, sp = 0.95, units = 10000)

##criando objeto com prevalencias e IC
epiprev.fendas.muni.nasc2015_2 <- epiprev.fendas.muni.nasc2015[["ap"]]

#unindo os dados de prevalencia
prev.fendas.muni.nasc2015 <- prev.fendas.muni.nasc2015 %>% 
  bind_cols(epiprev.fendas.muni.nasc2015_2) %>% 
  mutate(est = round(est, 2))
...
#unindo os dados de nascidos vivos e anomalias
prev.fendas.estab2015 <- fendas.estab2015 %>% 
  left_join(nv.estab2015, fendas.estab2015, by = "code_cnes") %>% 
  filter(str_detect(CodigoMunicipioNascimento,"42")) %>% 
  filter(code_cnes != is.na(code_cnes)) 

#calculando prevalencia
epiprev.fendas.estab2015 <-epi.prev(prev.fendas.estab2015$n, prev.fendas.estab2015$nasc_vivos, se = 0.95, sp = 0.95, units = 10000)

##criando objeto com prevalencias e IC
epiprev.fendas.estab2015_2 <- epiprev.fendas.estab2015[["ap"]]

#unindo os dados de prevalencia
prev.fendas.estab2015 <- prev.fendas.estab2015 %>% 
  bind_cols(epiprev.fendas.estab2015_2) %>% 
  mutate(est = round(est, 2))
...
## ---- mapa_fendas.muni.resid2015 --------
##MAPA ANOMALIAS SELECIONADAS POR MUNICIPIO DE RESIDENCIA
# unindo os dados com a base de mapa
dados_fendas.muni.resid2015 <- dplyr::left_join(geoMunicipiosSC, fendas.muni.resid2015, by = "code_muni")


#plotando o mapa
mapa_fendas.muni.resid2015 <- leaflet(data = dados_fendas.muni.resid2015) 

label_fendas.muni.resid2015 <- dados_fendas.muni.resid2015 %>%
  mutate (n = replace_na(n, 0))

colormagma_fendas.muni.resid2015 = colorNumeric("magma", domain = dados_fendas.muni.resid2015$n, alpha = TRUE, reverse =  TRUE)

mapa_fendas.muni.resid2015 %>% 
  addPolygons (weight = 0.5, fillColor = ~colormagma_fendas.muni.resid2015(n),
               ...
               label = sprintf("<p><strong>Município de residência: </strong>%s<br/><strong>
                               ...
                               label_fendas.muni.resid2015$name_muni, label_fendas.muni.resid2015$n) %>% 
                 lapply(htmltools::HTML),
               labelOptions = labelOptions(textsize = "14px",
                                           opacity = 0.8, direction = "bottom")) %>%
  addLegend(position = "bottomleft", pal = colormagma_fendas.muni.resid2015, values = ~n, na.label = "Sem registro", opacity = 1, title = "Nascidos vivos <br> com anomalias")



## ---- mapa_fendas.macro.resid2015 --------
##MAPA ANOMALIAS SELECIONADAS POR MACRORREGIAO DE RESIDENCIA
# unindo os dados com a base de mapa
dados_fendas.macro.resid2015 <- dplyr::left_join(geoMacrorregioesSC, fendas.macro.resid2015, by = "code_health_marcroregion")


#plotando o mapa
mapa_fendas.macro.resid2015 <- leaflet(data = dados_fendas.macro.resid2015) 

label_fendas.macro.resid2015 <- dados_fendas.macro.resid2015 %>%
  mutate (n = replace_na(n, 0)) %>% 
  mutate(name_health_macroregion = str_to_title(name_health_macroregion))

label_fendas.macro.resid2015["3", "name_health_macroregion"] <- "Meio Oeste E Serra Catarinense"

colormagma_fendas.macro.resid2015  = colorNumeric("magma", domain = dados_fendas.macro.resid2015$n, alpha = TRUE, reverse =  TRUE)

mapa_fendas.macro.resid2015 %>% 
  addPolygons (weight = 0.5, fillColor = ~colormagma_fendas.macro.resid2015(n),
               color = "black", fillOpacity = 1.0,
               smoothFactor = 1,
               highlightOptions = highlightOptions(weight = 3, bringToFront = TRUE),
               label = sprintf("<p><strong>Macrorregião de saúde: </strong>%s<br/><strong>
                               Nascidos vivos com anomalias: </strong>%s", 
                               label_fendas.macro.resid2015$name_health_macroregion, label_fendas.macro.resid2015$n) %>% 
                 lapply(htmltools::HTML),
               labelOptions = labelOptions(textsize = "14px",
                                           opacity = 0.8, direction = "bottom")) %>%
  addLegend(position = "bottomleft", pal = colormagma_fendas.macro.resid2015, values = ~n, na.label = "Sem registro", opacity = 1, title = "Nascidos vivos <br> com anomalias")


## ---- mapa_fendas.reg.resid2015 --------
##MAPA ANOMALIAS SELECIONADAS POR REGIAO DE RESIDENCIA
# unindo os dados com a base de mapa
dados_fendas.reg.resid2015 <- dplyr::left_join(geoRegioesSC, fendas.reg.resid2015, by = "code_health_region")


#plotando o mapa
mapa_fendas.reg.resid2015 <- leaflet(data = dados_fendas.reg.resid2015) 

label_fendas.reg.resid2015 <- dados_fendas.reg.resid2015 %>%
  mutate (n = replace_na(n, 0)) 

colormagma_fendas.reg.resid2015 = colorNumeric("magma", domain = dados_fendas.reg.resid2015$n, alpha = TRUE, reverse =  TRUE)

mapa_fendas.reg.resid2015 %>% 
  addPolygons (weight = 0.5, fillColor = ~colormagma_fendas.reg.resid2015(n),
               color = "black", fillOpacity = 1.0,
               smoothFactor = 1,
               highlightOptions = highlightOptions(weight = 3, bringToFront = TRUE),
               label = sprintf("<p><strong>Região de saúde: </strong>%s<br/><strong>
                               Nascidos vivos com anomalias: </strong>%s", 
                               label_fendas.reg.resid2015$name_health_region, label_fendas.reg.resid2015$n) %>% 
                 lapply(htmltools::HTML),
               labelOptions = labelOptions(textsize = "14px",
                                           opacity = 0.8, direction = "bottom")) %>%
  addLegend(position = "bottomleft", pal = colormagma_fendas.reg.resid2015, values = ~n, na.label = "Sem registro", opacity = 1, title = "Nascidos vivos <br> com anomalias")


## ---- mapa_fendas.muni.nasc2015 --------
##MAPA ANOMALIAS SELECIONADAS POR MUNICIPIO DE NASCIMENTO SC
# unindo os dados com a base de mapa
dados_fendas.muni.nasc2015 <- dplyr::left_join(geoMunicipiosSC, fendas.muni.nasc2015, by = "code_muni")


#plotando o mapa
mapa_fendas.muni.nasc2015 <- leaflet(data = dados_fendas.muni.nasc2015) 

label_fendas.muni.nasc2015 <- dados_fendas.muni.nasc2015 %>%
  mutate (n = replace_na(n, 0))

colormagma_fendas.muni.nasc2015 = colorNumeric("magma", domain = dados_fendas.muni.nasc2015$n, alpha = TRUE, reverse =  TRUE)

mapa_fendas.muni.nasc2015 %>% 
  addPolygons (weight = 0.5, fillColor = ~colormagma_fendas.muni.nasc2015(n),
               color = "black", fillOpacity = 1.0,
               smoothFactor = 1,
               highlightOptions = highlightOptions(weight = 3, bringToFront = TRUE),
               label = sprintf("<p><strong>Município de Nascimento: </strong>%s<br/><strong>
                               Nascidos vivos com anomalias: </strong>%s", 
                               label_fendas.muni.nasc2015$name_muni, label_fendas.muni.nasc2015$n) %>% 
                 lapply(htmltools::HTML),
               labelOptions = labelOptions(textsize = "14px",
                                           opacity = 0.8, direction = "bottom")) %>%
  addLegend(position = "bottomleft", pal = colormagma_fendas.muni.nasc2015, values = ~n, na.label = "Sem registro", opacity = 1, title = "Nascidos vivos <br> com anomalias")


## ---- mapa_fendas.estab2015 --------
...
# unindo os dados com a base de mapa
dados_fendas.estab2015 <- dplyr::left_join(geoEstabeleTodos, fendas.estab2015, by = "code_cnes")

#filtrando por estabelecimentos com anomalias
dados_fendas.estab2015 <- dados_fendas.estab2015 %>% 
  filter(n != is.na(n))
...
levels(dados_fendas.estab2015$Marcador) <- c("verde", "laranja", "vermelho")


#plotando o mapa  
label_fendas.estab2015 <- sprintf("<p><strong>Estabelecimento: </strong>%s<br/>
                 <strong>Nascidos vivos com anomalias: </strong>%s", dados_fendas.estab2015$Estabelecimento,
                                  dados_fendas.estab2015$n) %>% lapply(htmltools::HTML)


mapa_fendas.estab2015 <- leaflet(dados_fendas.estab2015) %>% addTiles() 
mapa_fendas.estab2015 %>% 
  addMarkers(icon = ~marcadores_personalizados[as.numeric(Marcador)],
             label = label_fendas.estab2015,  
             labelOptions = labelOptions(textsize = "14px", opacity = 0.7, direction = "top"),
             options = markerOptions(riseOnHover = TRUE)) %>% 
  addLegendImage(mapa_fendas.estab2015, images = c("/Users/Laysa/Google Drive/Doutorado PPGBM/Análises do banco/Marcador verde.png",
...
## ---- mapa_prev.fendas.muni.resid2015 --------
### MAPA PREVALENCIA POR MUNICIPIOS DE RESIDENCIA DA MAE
# unindo os dados com a base de mapa
dados_prev.fendas.muni.resid2015 <- dplyr::left_join(geoMunicipiosSC, prev.fendas.muni.resid2015, by = "code_muni") 

#modificando decimal no label e susbtituindo NAs
label_prev.fendas.muni.resid2015 <- dados_prev.fendas.muni.resid2015 %>%
  mutate (est = replace_na(est, 0.00)) %>%
  mutate (est = scales::number(est, accuracy = 0.01, big.mark = ".", decimal.mark = ","))

#plotando o mapa
mapa_prev.fendas.muni.resid2015  <- leaflet(data = dados_prev.fendas.muni.resid2015) 

colormagma_prev.fendas.muni.resid2015 = colorNumeric("magma", domain = dados_prev.fendas.muni.resid2015$est, alpha = TRUE, reverse =  TRUE)

mapa_prev.fendas.muni.resid2015 %>% 
  addPolygons (weight = 0.5, fillColor = ~colormagma_prev.fendas.muni.resid2015(est),
               color = "black", fillOpacity = 1.0,
               smoothFactor = 1,
               highlightOptions = highlightOptions(weight = 3, bringToFront = TRUE),
               label = sprintf("<p><strong>Município de residência: </strong>%s<br/><strong>
                               Prevalência/10.000: </strong>%s", 
                               label_prev.fendas.muni.resid2015$name_muni, label_prev.fendas.muni.resid2015$est) %>% 
                 lapply(htmltools::HTML),
               labelOptions = labelOptions(textsize = "14px",
                                           opacity = 0.8, direction = "bottom")) %>%
  addLegend(position = "bottomleft", pal = colormagma_prev.fendas.muni.resid2015, values = ~est, na.label = "Sem registro", opacity = 1, labFormat = labelFormat(big.mark = "."), title = "Prevalência/10.000")


## ---- mapa_prev.fendas.macro.resid2015 --------
### MAPA PREVALENCIA POR MACRORREGIAO DE RESIDENCIA DA MAE
# unindo os dados com a base de mapa
dados_prev.fendas.macro.resid2015 <- dplyr::left_join(geoMacrorregioesSC, prev.fendas.macro.resid2015, by = "code_health_marcroregion") 

#modificando decimal no label e susbtituindo NAs
label_prev.fendas.macro.resid2015 <- dados_prev.fendas.macro.resid2015 %>%
  mutate (est = scales::number(est, accuracy = 0.01, big.mark = ".", decimal.mark = ",")) %>% 
  mutate(name_health_macroregion = str_to_title(name_health_macroregion))

label_prev.fendas.macro.resid2015["3", "name_health_macroregion"] <- "Meio Oeste E Serra Catarinense"

#plotando o mapa
mapa_prev.fendas.macro.resid2015  <- leaflet(data = dados_prev.fendas.macro.resid2015) 

colormagma_prev.fendas.macro.resid2015 = colorNumeric("magma", domain = dados_prev.fendas.macro.resid2015$est, alpha = TRUE, reverse =  TRUE)

mapa_prev.fendas.macro.resid2015 %>% 
  addPolygons (weight = 0.5, fillColor = ~colormagma_prev.fendas.macro.resid2015(est),
               color = "black", fillOpacity = 1.0,
               smoothFactor = 1,
               highlightOptions = highlightOptions(weight = 3, bringToFront = TRUE),
               label = sprintf("<p><strong>Macrorregião de saúde: </strong>%s<br/>
               <strong>Prevalência/10.000: </strong>%s", label_prev.fendas.macro.resid2015$name_health_macroregion, label_prev.fendas.macro.resid2015$est) %>% 
                 lapply(htmltools::HTML),
               labelOptions = labelOptions(textsize = "14px",
                                           opacity = 0.8, direction = "bottom")) %>%
  addLegend(position = "bottomleft", pal = colormagma_prev.fendas.macro.resid2015, values = ~est, na.label = "Sem registro", opacity = 1, labFormat = labelFormat(big.mark = "."), title = "Prevalência/10.000")


## ---- mapa_prev.fendas.reg.resid2015 --------
### MAPA PREVALENCIA POR REGIAO DE RESIDENCIA DA MAE
# unindo os dados com a base de mapa
dados_prev.fendas.reg.resid2015 <- dplyr::left_join(geoRegioesSC, prev.fendas.reg.resid2015, by = "code_health_region") 

#modificando decimal no label e susbtituindo NAs
label_prev.fendas.reg.resid2015 <- dados_prev.fendas.reg.resid2015 %>%
  mutate (est = replace_na(est, 0.00)) %>%
  mutate (est = scales::number(est, accuracy = 0.01, big.mark = ".", decimal.mark = ","))


#plotando o mapa
mapa_prev.fendas.reg.resid2015  <- leaflet(data = dados_prev.fendas.reg.resid2015) 

colormagma_prev.fendas.reg.resid2015 = colorNumeric("magma", domain = dados_prev.fendas.reg.resid2015$est, alpha = TRUE, reverse =  TRUE)

mapa_prev.fendas.reg.resid2015 %>% 
  addPolygons (weight = 0.5, fillColor = ~colormagma_prev.fendas.reg.resid2015(est),
               color = "black", fillOpacity = 1.0,
               smoothFactor = 1,
               highlightOptions = highlightOptions(weight = 3, bringToFront = TRUE),
               label = sprintf("<p><strong>Região de saúde: </strong>%s<br/><strong>
                               Prevalência/10.000: </strong>%s", 
                               label_prev.fendas.reg.resid2015$name_health_region, label_prev.fendas.reg.resid2015$est) %>% 
                 lapply(htmltools::HTML),
               labelOptions = labelOptions(textsize = "14px",
                                           opacity = 0.8, direction = "bottom")) %>%
  addLegend(position = "bottomleft", pal = colormagma_prev.fendas.reg.resid2015, values = ~est, na.label = "Sem registro", opacity = 1, labFormat = labelFormat(big.mark = "."), title = "Prevalência/10.000")



## ---- mapa_prev.fendas.muni.nasc2015 --------
### MAPA PREVALENCIA POR MUNICIPIOS DE NASCIMENTO
# unindo os dados com a base de mapa
dados_prev.fendas.muni.nasc2015 <- dplyr::left_join(geoMunicipiosSC, prev.fendas.muni.nasc2015, by = "code_muni") 

#substituindo NAs no label
label_prev.fendas.muni.nasc2015 <- dados_prev.fendas.muni.nasc2015 %>%
  mutate (est = replace_na(est, 0.00))

#modificando decimal no label 
label_prev.fendas.muni.nasc2015 <- label_prev.fendas.muni.nasc2015  %>%
  mutate (est = scales::number(est, accuracy = 0.01, big.mark = ".", decimal.mark = ","))


#plotando o mapa
mapa_prev.fendas.muni.nasc2015  <- leaflet(data = dados_prev.fendas.muni.nasc2015) 

colormagma_prev.fendas.muni.nasc2015 = colorNumeric("magma", domain = dados_prev.fendas.muni.nasc2015$est, alpha = TRUE, reverse =  TRUE)

mapa_prev.fendas.muni.nasc2015 %>% 
  addPolygons (weight = 0.5, fillColor = ~colormagma_prev.fendas.muni.nasc2015(est),
               color = "black", fillOpacity = 1.0,
               smoothFactor = 1,
               highlightOptions = highlightOptions(weight = 3, bringToFront = TRUE),
               label = sprintf("<p><strong>Município de nascimento: </strong>%s<br/><strong>
                               Prevalência/10.000: </strong>%s", 
                               label_prev.fendas.muni.nasc2015$name_muni, label_prev.fendas.muni.nasc2015$est) %>% 
                 lapply(htmltools::HTML),
               labelOptions = labelOptions(textsize = "14px",
                                           opacity = 0.8, direction = "bottom")) %>%
  addLegend(position = "bottomleft", pal = colormagma_prev.fendas.muni.nasc2015, values = ~est, na.label = "Sem registro", opacity = 1, labFormat = labelFormat(big.mark = "."), title = "Prevalência/10.000")


## ---- mapa_prev.fendas.estab2015 --------
### MAPA PREVALENCIAS POR ESTABELECIMENTO
#icones dos marcadores criados anteriormente
# unindo os dados com a base de mapa
dados_prev.fendas.estab2015 <- dplyr::left_join(geoEstabeleTodos, prev.fendas.estab2015, by = "code_cnes")

#filtrando por estabelecimentos com anomalias em santa catarina
dados_prev.fendas.estab2015 <- dados_prev.fendas.estab2015 %>% 
  filter(abbrev_state == "SC") %>% 
  filter(n != is.na(n))
...
#organizando nivel marcadores
levels(dados_prev.fendas.estab2015$Marcador) <- c("verde", "laranja", "vermelho")


#criando label e modificando decimal
label_prev.fendas.estab2015 <- dados_prev.fendas.estab2015  %>%
  mutate (est = scales::number(est, accuracy = 0.01, big.mark = ".", decimal.mark = ","))


label_prev.fendas.estab2015 <- sprintf("<p><strong>Estabelecimento: </strong>%s<br/>
                 <strong>Prevalência/10.000: </strong>%s", label_prev.fendas.estab2015$Estabelecimento,
                                       label_prev.fendas.estab2015$est) %>% lapply(htmltools::HTML)

#plotando o mapa
mapa_prev.fendas.estab2015 <- leaflet(dados_prev.fendas.estab2015) %>% addTiles() 
mapa_prev.fendas.estab2015 %>% 
  addMarkers(icon = ~marcadores_personalizados[as.numeric(Marcador)],
             label = label_prev.fendas.estab2015,  
             labelOptions = labelOptions(textsize = "14px", opacity = 0.7, direction = "top"),
             options = markerOptions(riseOnHover = TRUE)) %>% 
  addLegendImage(mapa_prev.fendas.estab2015, images = c("/Users/Laysa/Google Drive/Doutorado PPGBM/Análises do banco/Marcador verde.png",
                                                        "/Users/Laysa/Google Drive/Doutorado PPGBM/Análises do banco/Marcador laranja.png",
                                                        "/Users/Laysa/Google Drive/Doutorado PPGBM/Análises do banco/Marcador vermelho.png"),
...
## ---- tab_prev.fendas.muni.resid2015 --------
tab_prev.fendas.muni.resid2015 <- prev.fendas.muni.resid2015 %>%
  rename(CodigoMunicipioResidencia = code_muni) 

tab_prev.fendas.muni.resid2015 <- tab_prev.fendas.muni.resid2015 %>% 
  left_join(municipiosresidencia, tab_prev.fendas.muni.resid2015, by = "CodigoMunicipioResidencia") %>% 
  mutate (est = scales::number(est, accuracy = 0.01, big.mark = ".", decimal.mark = ",")) %>% 
...
## ---- tab_prev.fendas.macro.resid2015 --------
tab_prev.fendas.macro.resid2015 <- prev.fendas.macro.resid2015 %>%
  rename(CodigoMacrorregiaoSaude = code_health_marcroregion) %>% 
  mutate(CodigoMacrorregiaoSaude = as.numeric(CodigoMacrorregiaoSaude))

tab_prev.fendas.macro.resid2015 <- tab_prev.fendas.macro.resid2015 %>% 
  left_join(macrorregioes, tab_prev.fendas.macro.resid2015, by = "CodigoMacrorregiaoSaude") %>% 
  mutate (est = scales::number(est, accuracy = 0.01, big.mark = ".", decimal.mark = ",")) %>% 
  mutate (lower = scales::number(lower, accuracy = 0.01, big.mark = ".", decimal.mark = ",")) %>% 
  mutate (upper = scales::number(upper, accuracy = 0.01, big.mark = ".", decimal.mark = ",")) %>%
  unite (col = IC95, lower:upper, sep = "-") %>%
...
## ---- tab_prev.fendas.reg.resid2015 --------
tab_prev.fendas.reg.resid2015 <- prev.fendas.reg.resid2015 %>%
  rename(CodigoRegiaoSaude = code_health_region) %>% 
  mutate(CodigoRegiaoSaude = as.numeric(CodigoRegiaoSaude)) 

tab_prev.fendas.reg.resid2015 <- tab_prev.fendas.reg.resid2015 %>% 
  left_join(regioes, tab_prev.fendas.reg.resid2015, by = "CodigoRegiaoSaude") %>% 
  mutate (est = scales::number(est, accuracy = 0.01, big.mark = ".", decimal.mark = ",")) %>% 
  mutate (lower = scales::number(lower, accuracy = 0.01, big.mark = ".", decimal.mark = ",")) %>% 
  mutate (upper = scales::number(upper, accuracy = 0.01, big.mark = ".", decimal.mark = ",")) %>%
  unite (col = IC95, lower:upper, sep = "-") %>%
...
## ---- tab_prev.fendas.muni.nasc2015 --------
tab_prev.fendas.muni.nasc2015 <- prev.fendas.muni.nasc2015 %>%
  rename(CodigoMunicipioNascimento = code_muni) 

tab_prev.fendas.muni.nasc2015 <- tab_prev.fendas.muni.nasc2015 %>% 
  left_join(municipios, tab_prev.fendas.muni.nasc2015, by = "CodigoMunicipioNascimento") %>% 
  mutate (est = scales::number(est, accuracy = 0.01, big.mark = ".", decimal.mark = ",")) %>% 
  mutate (lower = scales::number(lower, accuracy = 0.01, big.mark = ".", decimal.mark = ",")) %>% 
  mutate (upper = scales::number(upper, accuracy = 0.01, big.mark = ".", decimal.mark = ",")) %>%
  unite (col = IC95, lower:upper, sep = "-") %>%
...
## ---- tab_prev.fendas.estab2015 --------
tab_prev.fendas.estab2015 <- prev.fendas.estab2015 %>%
...
tab_prev.fendas.estab2015 <- tab_prev.fendas.estab2015 %>% 
  left_join(estabelecimentoC, tab_prev.fendas.estab2015, by = "CodigoEstabelecimento") %>% 
  mutate (est = scales::number(est, accuracy = 0.01, big.mark = ".", decimal.mark = ",")) %>% 
  mutate (lower = scales::number(lower, accuracy = 0.01, big.mark = ".", decimal.mark = ",")) %>% 
  mutate (upper = scales::number(upper, accuracy = 0.01, big.mark = ".", decimal.mark = ",")) %>%
  unite (col = IC95, lower:upper, sep = "-") %>%
...
tab_prev.fendas.estab2015 <- tab_prev.fendas.estab2015 %>%
  ...
  left_join(municipios, tab_prev.fendas.estab2015, by = "CodigoMunicipioNascimento") %>% 
  relocate(MunicipioNascimento, .after = CodigoMunicipioNascimento) %>% 
  mutate (UFNascimento = NULL)
